		...
		if (email_type != '') {
			...
			if(smodule == "Quotes"){
				...
				jQuery.ajax({
					...
					'async': false,
					success: function(data) {
						console.log("validate_sendPDFmail: " + data);
						var allegato_principale = "";
						var altri_allegati = "";
						var risultato = "";
						if(data.indexOf("--SPLIT--") != -1 && data.indexOf("--SPLIT--") != "-1"){
							var split_data = data.split("--SPLIT--");
							var allegato_principale = "";
							var altri_allegati = "";
							for(var i = 0; i < split_data.length; i++){
								if(i == 0){
									allegato_principale = split_data[i];
									risultato = risultato + "Principale: " + allegato_principale + "\n";
								}
								else if(i == 1){
									altri_allegati += split_data[i]; 
									risultato = risultato + "Secondari \n";
									risultato = risultato + i + ") " + split_data[i] + "\n";
								}
								else{
									altri_allegati += '--'+split_data[i]; 
									risultato = risultato + i + ") " + split_data[i] + "\n";
