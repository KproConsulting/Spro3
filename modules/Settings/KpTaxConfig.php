...
$tax_details = $InventoryUtils->getAllTaxes(); //kpro@tom101220181102
//$tax_details = $InventoryUtils->getAllTaxes('all','prod'); //kpro@tom101220181102
...
if($_REQUEST['save_tax'] == 'true')
{
	for($i=0;$i<count($tax_details);$i++)
	{
		...
		$codice_iva[ $tax_details[$i]['taxid'] ] = $_REQUEST[ $tax_details[$i]['taxname']."_codice_iva" ];
		$natura[ $tax_details[$i]['taxid'] ] = $_REQUEST[ $tax_details[$i]['taxname']."_natura" ];
		$normativa[ $tax_details[$i]['taxid'] ] = $_REQUEST[ $tax_details[$i]['taxname']."_norma" ];
	...
	/* kpro@tom101220181102 */
	kpUpdateTaxCodiceIva($codice_iva);
	kpUpdateTaxNatura($natura);
	kpUpdateTaxNorma($normativa);
	/* kpro@tom101220181102 end */
...
$smarty->display("Settings/KpTaxConfig.tpl");

function kpUpdateTaxCodiceIva($codice_iva, $sh=''){
	global $adb, $log, $table_prefix;
	$log->debug("Entering into the function kpUpdateTaxCodiceIva");
	
	foreach($codice_iva as $taxid => $new_val){
		if($sh != '' && $sh == 'sh'){

		}
		else{
			$query = "update ".$table_prefix."_inventorytaxinfo set kp_codice_iva = '".$new_val."' where taxid = ".$taxid;
		}
		$adb->query($query);
	}

	$log->debug("Exiting from the function kpUpdateTaxCodiceIva");
}

function kpUpdateTaxNatura($natura, $sh=''){
	global $adb, $log, $table_prefix;
	$log->debug("Entering into the function kpUpdateTaxNatura");
	
	foreach($natura as $taxid => $new_val){
		if($sh != '' && $sh == 'sh'){

		}
		else{
			$query = "update ".$table_prefix."_inventorytaxinfo set kp_natura = '".$new_val."' where taxid = ".$taxid;
		}
		$adb->query($query);
	}

	$log->debug("Exiting from the function kpUpdateTaxNatura");
}

function kpUpdateTaxNorma($norma, $sh=''){
	global $adb, $log, $table_prefix;
	$log->debug("Entering into the function kpUpdateTaxNorma");

	foreach($norma as $taxid => $new_val){
		if($sh != '' && $sh == 'sh'){

		}
		else{
			$query = "update ".$table_prefix."_inventorytaxinfo set kp_norma = '".$new_val."' where taxid = ".$taxid;
		}
		$adb->query($query);
	}

	$log->debug("Exiting from the function kpUpdateTaxNorma");
}
	...
	{	
		...
		if($sh != '' && $sh == 'sh'){
			$query1 = "insert into ".$table_prefix."_shippingtaxinfo values
						(".$taxid.", '".$taxname."', '".$taxlabel."', ".$taxvalue.", 0)";
		}
		else{
			$query1 = "INSERT INTO ".$table_prefix."_inventorytaxinfo 
						(taxid, taxname, taxlabel, percentage, deleted, kp_codice_iva, kp_natura, kp_norma)
						VALUES
						(".$taxid.", '".$taxname."', '".$taxlabel."', ".$taxvalue.", 0, '', '', '')";
		}
		/* kpro@tom101220181102 end */
