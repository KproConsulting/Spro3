...
class GDPRWS extends SDKExtendableUniqueClass {
	...
	public function __construct() {
		...
		$config = $vteProp->get('services.gdpr.config');
		...
		$templates = $vteProp->get('services.gdpr.templates');
	...
	public function install() {
		...
		$prop = $vteProp->get('services.gdpr.config');
		...
		if ($prop === null) {
			...
			$companyDetailsResult = $adb->limitQuery("SELECT logoname FROM {$table_prefix}_organizationdetails", 0, 1);
			...
			$prop = $vteProp->set('services.gdpr.config', $config);
		}
		
		$prop = $vteProp->get('services.gdpr.templates');
		...
		if ($prop === null) {
			...
			$prop = $vteProp->set('services.gdpr.templates', $templates);
		}
		
		$PPU = PrivacyPolicyUtils::getInstance();
		$ok = $PPU->save('Company', file_get_contents('include/utils/GDPRWS/templates/privacy_policy.html'));
		...
		foreach ($gdprModules as $module) {
			...
			foreach ($GDPRFields as $field) {
				...
				$fields[] = array('module' => $module, 'block' => 'LBL_GDPR_INFORMATION', 'name' => $field, 'label' => $label, 'uitype' => '56', 'readonly' => '99', 'columntype' => 'CHAR(1)', 'typeofdata' => 'C~O');
				$fields[] = array('module' => $module, 'block' => 'LBL_GDPR_INFORMATION', 'name' => $field.'_checkedtime', 'label' => $label.'_CHECKEDTIME', 'uitype' => '1', 'readonly' => '99', 'columntype' => 'DATETIME', 'typeofdata' => 'C~O');
				$fields[] = array('module' => $module, 'block' => 'LBL_GDPR_INFORMATION', 'name' => $field.'_remote_addr', 'label' => $label.'_REMOTE_ADDRESS', 'uitype' => '1', 'readonly' => '99', 'columntype' => 'C(45)', 'typeofdata' => 'C~O');
			}
			
			$fields[] = array('module' => $module, 'block' => 'LBL_GDPR_INFORMATION', 'name' => 'gdpr_sentdate', 'label' => 'LBL_GDPR_SENTTIME', 'uitype' => '1', 'readonly' => '99', 'columntype' => 'DATETIME', 'typeofdata' => 'C~O');
		...
		if (Vtecrm_Event::hasSupport()) {
			Vtecrm_Event::register('', 'vtiger.entity.beforesave', 'GDPRHandler', 'include/utils/GDPRWS/handlers/GDPRHandler.php');
		}
		...
		Vtiger_Link::addLink($contactsInstance->id, 'DETAILVIEWBASIC', 'LBL_GDPR_ANONYMIZE', 'javascript:gdprAnonymize(\'$MODULE$\',$RECORD$);');
		
		$leadsInstance = Vtiger_Module::getInstance('Leads');
		Vtiger_Link::addLink($leadsInstance->id, 'DETAILVIEWBASIC', 'LBL_GDPR_ANONYMIZE', 'javascript:gdprAnonymize(\'$MODULE$\',$RECORD$);');
	...
	protected function createTemplateEmail($name, $label, $subject, $description, $language, $type = 'Email') {
	...
	public function checkTables() {
		...
		if (!Vtiger_Utils::CheckTable($this->authTable)) {
		...
		if (!Vtiger_Utils::CheckTable($this->confirmTable)) {
			$schema_obj = new adoSchema($adb->database);
			$schema_obj->ExecuteSchema($schema_obj->ParseSchemaString($schema_table));
		}
	}
	...
	public function validateContactId($cid, &$error) {
		...
		$availableData['gdpr_marketing'] = $newsletterFocus->receivingNewsletter($email);
	...
	public function mergeContactData($accesstoken, $maincontact, $otherids, $options = array()) {
		...
		$changeLogQuery = "SELECT parent_id, changelogid, {$table_prefix}_changelog.description, modified_date
		...
		WHERE parent_id IN (".generateQuestionMarks($otherids).") AND {$table_prefix}_changelog.description NOT LIKE '%_Relation%'
		...
		foreach ($duplicates as $duplicate) {
			...
			foreach ($colFields as $key) {
				if (!in_array($key, array('firstname', 'lastname', 'email'))) {
	...
	public function sendSupportRequest($cid, $subject, $description, $options = array()) {
		...
		return $status ? $this->success($output) : $this->error('SEND_EMAIL_FAILED');
	}
	...
	public function getPrivacyPolicy($cid, $options = array()) {
		...
		$PPU = PrivacyPolicyUtils::getInstance();
		$companyPrivacyPolicy = $PPU->get('Company');
		
		$output = $this->formatOutputData(array('privacy_policy' => $companyPrivacyPolicy), $contactData, $options);
	...
	public function sendPrivacyPolicy($cid, $options = array()) {
		...
		return $success ? $this->success($output) : $this->error('SEND_EMAIL_FAILED');
	}
	...
	protected function calculateFieldStructure($module) {
		...
		foreach ($blocks as $blockid => $binfo) {
			usort($blocks[$blockid]['fields'], create_function('$v1,$v2', 'return ($v1["sequence"] > $v2["sequence"] ? +1 : ($v1["sequence"] < $v2["sequence"] ? -1 : 0));'));
			$blocks[$blockid]['fields'] = $blocks[$blockid]['fields'];
		}
		
		$structure = array_values($blocks);
		
		usort($structure, create_function('$v1,$v2', 'return ($v1["sequence"] > $v2["sequence"] ? +1 : ($v1["sequence"] < $v2["sequence"] ? -1 : 0));'));
	...
	public function applyContactDelete($contactData, $accesstoken, $data) {
		...
		$focus = CRMEntity::getInstance($module);
		$err = $focus->retrieve_entity_info_no_html($contactid, $module, false); //mycrmv@182782
		$focus->mode = 'edit';
		$focus->id = $contactid;
		...
		$changeLogs = $RM->getRelatedIds($module, $contactid, array('ChangeLog'));
		foreach ($changeLogs as $changeLog) {
			$changeLogFocus = CRMEntity::getInstance('ChangeLog');
			$changeLogFocus->retrieve_entity_info_no_html($changeLog, 'ChangeLog', false);
			$changeLogFocus->mode = 'edit';
			$changeLogFocus->id = $changeLog;
			$changeLogFocus->mark_deleted($changeLog);
		}
	...
	public function applyContactMerge($contactData, $accesstoken, $data) {
		...
		foreach ($transferRelations as $transferid) {
			$relatedIds = $RM->getRelatedIds($module, $transferid, array(), array('ChangeLog'));
	...
	protected function formatOutputData($data, $contactData, $options = array()) {
		...
		if ($options && $options['include_contacts_data']) {
			...
			$cid = urlencode(base64_encode("C|".$contactEmail."|".$module));
			
			$duplicates = $this->getContactDuplicates($module, $contactid, $column, $value);
			
			$out = array_merge(array('cid' => $cid, 'email' => $contactEmail, 'contact' => $contactData['available_data'], 'duplicates' => $duplicates), $out);
		}
