...
// kpro@tom101220181102 - Tag per la fatturazione elettronica (gestione tassazione)
	...
	*/
	...
	function getAllTaxes($available='all', $sh='',$mode='',$id='') {
		...
		for ($i=0;$i<$noofrows;++$i) {
			...
			/*$kp_log = '/var/www/html/spro_dev/log_marco.txt';
			file_put_contents($kp_log, "select * from $tablename  where $where_ids  deleted=0 order by taxid");*/
			...
			if($sh != '' && $sh == 'sh') {
				...
				$kp_codice_iva = $adb->query_result($res, $i, 'kp_codice_iva');
				$kp_codice_iva = html_entity_decode(strip_tags($kp_codice_iva), ENT_QUOTES, $default_charset);

				$kp_natura = $adb->query_result($res, $i, 'kp_natura');
				$kp_natura = html_entity_decode(strip_tags($kp_natura), ENT_QUOTES, $default_charset);

				$kp_norma = $adb->query_result($res, $i, 'kp_norma');
				$kp_norma = html_entity_decode(strip_tags($kp_norma), ENT_QUOTES, $default_charset);
			...
			$taxtypes[$i]['kp_natura'] = $adb->query_result($res, $i, 'kp_natura');
			$taxtypes[$i]['kp_norma'] = $adb->query_result($res, $i, 'kp_norma');
			$taxtypes[$i]['kp_natura_decode'] = $kp_natura_decode;
		...
		/* kpro@tom101220181102 */
		//Un semplice print per log
		/*if( $sh != 'sh' ){
			$kp_log = 'log_marco.txt';
			file_put_contents($kp_log, print_r($taxtypes, true));
		}*/
		/* kpro@tom101220181102 end */
	...
	//Non più necessaria: pre ripristinarla bisogna rinominarla in getAllTaxes (ovviamente editando la funzione getAllTaxes standard)
	function KpGetAllTaxes($available='all', $sh='',$mode='',$id='') {
		global $adb, $log, $table_prefix, $default_charset;
		$log->debug("Entering into the function getAllTaxes($available,$sh,$mode,$id)");
		$taxtypes = Array();
		if($sh != '' && $sh == 'sh') {
		...
		if ($mode == 'edit' && $id != '' ) {
			...
			elseif($sh == 'prod'){
				...
				$query_taxes = "SELECT taxname,
								taxid
								FROM $tablename
								LEFT JOIN kp_settings_tasse ON id_tassa = taxid
	...
	function deleteInventoryProductDetails($focus)	{
		...
		/* kpro@tom101220181102 */
		$adb->pquery("DELETE FROM kp_inventoryproductrel WHERE id=?", array($focus->id));
		/* kpro@tom101220181102 end */
	...
	function updateInventoryProductRel($entity)	{
		...
		/* kpro@tom101220181102 */
		$adb->pquery("UPDATE kp_inventoryproductrel SET incrementondel=1 WHERE id=?",array($entity_id));
		/* kpro@tom101220181102 end */
		...
		/* kpro@tom101220181102 */
		$this->kpUpdateInventoryProductRel($entity_id);
		/* kpro@tom101220181102 end */
...
//			} //crmv@43358
	...
	//Non più necessaria: pre ripristinarla bisogna rinominarla in calcInventoryTotals (ovviamente editando la funzione calcInventoryTotals standard)
	function KpCalcInventoryTotals($totalinfo) {
		$workingPrecision = $this->workingPrecision;
		$outputPrecision = $this->outputPrecision;
...
//			} //crmv@43358
			...
			if($_REQUEST["deleted".$i] == 1)
				...
				if(isset($_REQUEST['productDescription'.$i]))
					...
					if (!$returnTotals) {
						...
						$kp_id = $focus->id;
						$kp_productid = $prod_id;
						$kp_relmodule = $module;
						$kp_lineitem_id = $lineitem_id;
						$kp_rif_ord_cliente = "";
						$kp_data_ord_cliente = "";
						$kp_codice_cup = "";
						$kp_codice_cig = "";
						...
						if( isset($_REQUEST['kpTaxName'.$i]) ){
							$kp_id_tassa_temp = $_REQUEST['kpTaxName'.$i];
							if( $kp_id_tassa_temp != '' ){
								$kp_id_tassa_temp = explode("_", $kp_id_tassa_temp);
								$kp_id_tassa = $kp_id_tassa_temp[0];
							}
						}

						if( isset($_REQUEST['kpRifOrdineCliente'.$i]) ){
							$kp_rif_ord_cliente = $_REQUEST['kpRifOrdineCliente'.$i];
						}

						if( isset($_REQUEST['kpDataOrdineCliente'.$i]) ){
							$kp_data_ord_cliente = $_REQUEST['kpDataOrdineCliente'.$i];
						}

						if( isset($_REQUEST['kpCodiceCup'.$i]) ){
							$kp_codice_cup = $_REQUEST['kpCodiceCup'.$i];
						}

						if( isset($_REQUEST['kpCodiceCig'.$i]) ){
							$kp_codice_cig = $_REQUEST['kpCodiceCig'.$i];
						}

						$query = "INSERT INTO kp_inventoryproductrel (
									id,
									productid,
									relmodule,
									incrementondel,
						...
						/*$log_file = __DIR__.'/../../kp_log_file_saveInventoryProductDetails.txt';
	...
	function getAssociatedProducts($module,$focus,$seid='') {
		...
		/*$log_file = __DIR__.'/../../kp_log_file_saveInventoryProductDetails.txt';
		file_put_contents($log_file, print_r($product_Detail, true));*/
	...
	function processProductSqlRow($row, $index, $options, $module, &$focus) {
		...
		/* kpro@tom101220181102 */
		$kp_dati_custom = $this->kpGetDatiRigaCustom($row['productid'], $row['lineitem_id']);
		$kp_id_tassa_selezionata = $kp_dati_custom["id_tassa"];
		$kp_label_tassa_selezionata = $kp_dati_custom["taxlabel"];
		$kp_rif_ord_cliente = $kp_dati_custom["rif_ord_cliente"];
		$kp_data_ord_cliente = $kp_dati_custom["data_ord_cliente"];
		$kp_codice_cup = $kp_dati_custom["codice_cup"];
		$kp_codice_cig = $kp_dati_custom["codice_cig"];
		/* kpro@tom101220181102 end */
		...
		for ($tax_count=0; $tax_count<count($tax_details); ++$tax_count) {
			...
			if( $kp_id_tassa_selezionata != "" && $tax_name == $kp_id_tassa_selezionata){
				$output['taxes'][$tax_count]['kpTassaSelezionata'] = '1';
				$tax_info_message .= "$tax_label : ".$this->formatUserNumber($tax_value)."% = ".$this->formatUserNumber($prodPrices['taxes'][$tax_count]['amount'])." \\n";
			}
			else{
				$output['taxes'][$tax_count]['kpTassaSelezionata'] = '0';
			}
			$output['taxes'][$tax_count]['percentuale_default'] = $tax_details[$tax_count]['percentage'];
			/* kpro@tom101220181102 end */
		...
		/* kpro@tom101220181102 */
		$output['kpIdTassaRiga'.$i] = $kp_id_tassa_selezionata;
		$output['kpLabelTassaRiga'.$i] = $kp_label_tassa_selezionata;
		$output['kpRifOrdineCliente'.$i] = $kp_rif_ord_cliente;
		$output['kpDataOrdineCliente'.$i] = $kp_data_ord_cliente;
		$output['kpCodiceCup'.$i] = $kp_codice_cup;
		$output['kpCodiceCig'.$i] = $kp_codice_cig;

		//Semplice log
		/*$kp_log = '/var/www/html/spro_dev/log_marco.txt';
		file_put_contents($kp_log, print_r($output, true));*/
		/* kpro@tom101220181102 end */
	...
	//Non più necessaria: pre ripristinarla bisogna rinominarla in getFinalDetails (ovviamente editando la funzione getFinalDetails standard)
	function KpGetFinalDetails($module, $focus, $record=''){
	...
	/* kpro@tom101220181102 */
	function kpGetDatiRiga($id, $productid, $lineitem_id){
		...
		$query = "SELECT 
					...
					sequence_no
					FROM {$table_prefix}_inventoryproductrel
					WHERE id = ".$id." AND productid = ".$productid." AND lineitem_id = ".$lineitem_id;
		...
		if( $num_result > 0 ){
			...
			$kp_discount_percent = $adb->query_result($result_query, 0, 'discount_percent');
			$kp_discount_percent = html_entity_decode(strip_tags($kp_discount_percent), ENT_QUOTES, $default_charset);
			
			$kp_discount_amount = $adb->query_result($result_query, 0, 'discount_amount');
			$kp_discount_amount = html_entity_decode(strip_tags($kp_discount_amount), ENT_QUOTES, $default_charset);
			
			$kp_total_notaxes = $adb->query_result($result_query, 0, 'total_notaxes');
			$kp_total_notaxes = html_entity_decode(strip_tags($kp_total_notaxes), ENT_QUOTES, $default_charset);
			
			$kp_incrementondel = $adb->query_result($result_query, 0, 'incrementondel');
			$kp_incrementondel = html_entity_decode(strip_tags($kp_incrementondel), ENT_QUOTES, $default_charset);
			
			$kp_linetotal = $adb->query_result($result_query, 0, 'linetotal');
			$kp_linetotal = html_entity_decode(strip_tags($kp_linetotal), ENT_QUOTES, $default_charset);
			
			$kp_tax_total = $adb->query_result($result_query, 0, 'tax_total');
			$kp_tax_total = html_entity_decode(strip_tags($kp_tax_total), ENT_QUOTES, $default_charset);

			$kp_sequence_no = $adb->query_result($result_query, 0, 'sequence_no');
			$kp_sequence_no = html_entity_decode(strip_tags($kp_sequence_no), ENT_QUOTES, $default_charset);

		}
		else{
			$kp_discount_percent = 0;
			$kp_discount_amount = 0;
			$kp_total_notaxes = 0;
			$kp_incrementondel = 0;
			$kp_linetotal = 0;
			$kp_tax_total = 0;
		...
		$result = array("discount_percent" => $kp_discount_percent,
						"discount_amount" => $kp_discount_amount,
						"total_notaxes" => $kp_total_notaxes,
						"incrementondel" => $kp_incrementondel,
						"linetotal" => $kp_linetotal,
						"tax_total" => $kp_tax_total,
						"sequence_no" => $kp_sequence_no);

		return $result;

	}

	function kpGetDatiRigaCustom($productid, $lineitem_id){
		global $adb, $table_prefix, $default_charset;

		$result = "";

		$query = "SELECT 
					invrel.id_tassa id_tassa,
					invrel.rif_ord_cliente rif_ord_cliente,
					invrel.data_ord_cliente data_ord_cliente,
					invrel.codice_cup codice_cup,
					invrel.codice_cig codice_cig,
					...
					FROM kp_inventoryproductrel invrel
					LEFT JOIN vte_inventorytaxinfo taxinfo ON taxinfo.taxname = invrel.id_tassa
					WHERE invrel.productid = ".$productid." AND invrel.lineitem_id = ".$lineitem_id;
		...
		if( $num_result > 0 ){
			...
			$id_tassa = $adb->query_result($result_query, 0, 'id_tassa');
		...
		$result = array("id_tassa" => $id_tassa,
						"taxlabel" => $taxlabel,
						"percentage" => $percentage,
						"codice_iva" => $codice_iva,
						"natura" => $natura,
						"norma" => $norma,
						"rif_ord_cliente" => $rif_ord_cliente,
	...
	function kpUpdateInventoryProductRel($id){
		global $adb, $table_prefix, $default_charset;

		$lista_righe = $this->kpListaInventoryProductRel($id);

		foreach( $lista_righe as $riga ){

			$kp_id_tassa = $_REQUEST['kpTaxName'.$riga["sequence_no"]];

			$update = "UPDATE kp_inventoryproductrel
						productid = ".$riga["productid"].",
						incrementondel = ".$riga["incrementondel"].",
						codice_tassa = '".$kp_id_tassa."'
						WHERE id = ".$id." AND lineitem_id = ".$riga["lineitem_id"];
	...
	function kpListaInventoryProductRel($id){
		global $adb, $table_prefix, $default_charset;
		...
		for($i = 0; $i < $num_result; $i++){
			...
			$incrementondel = $adb->query_result($result_query, $i, 'incrementondel');
			$incrementondel = html_entity_decode(strip_tags($incrementondel), ENT_QUOTES, $default_charset);
